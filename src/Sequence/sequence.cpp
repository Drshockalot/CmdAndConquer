...
sequence::sequence ()
{
	record_action((action)action_invalid, 0);
...
{
	if(insert_worker(index, buf, length, (action)action_insert))
	{
		record_action((action)action_insert, index + length);
		return true;
	}
	else
	{
...
bool sequence::erase_worker (size_w index, size_w length, action act)
{
	...
	//
	else if(index + length == spanindex + sptr->length && can_optimize((action)action_erase, index+length))
	{
...
{
	if(erase_worker(index, len, (action)action_erase))
	{
		record_action((action)action_erase, index);
		return true;
	}
	else
	{
...
bool sequence::replace(size_w index, const seqchar *buf, size_w length, size_w erase_length)
{
	...
	if(remlen > 0 && index < sequence_length && !erase_worker(index, remlen, (action)action_replace))
	{
		ungroup();
		return false;
	}
	...
	if(insert_worker(index, buf, length, (action)action_replace))
	{
		ungroup();
		record_action((action)action_replace, index + length);
		return true;
	}
	else
	{
...
{
	lastaction = (action)action_invalid;
}